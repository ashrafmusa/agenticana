...
export class AgenticanaSidebarProvider implements vscode.WebviewViewProvider {
    public static readonly viewType = 'agenticana.sidebarView';
    ...
    private async _sendState(webview: vscode.Webview) {
        const root = this._getProjectRoot();
        ...
        let lastEvolution: Record<string, unknown> = {};
        try {
        ...
        try {
            ...
            gapCount = intel.reduce((sum: number, r: Record<string, unknown[]>) =>
                sum + (r.trending_requests?.length || 0), 0);
        } catch {}

        // Recent guardian audit logs
        let recentAudits: { timestamp: string; blocked: boolean }[] = [];
        try {
            ...
            if (fs.existsSync(logDir)) {
                recentAudits = fs.readdirSync(logDir)
                    ...
                    .filter(Boolean) as { timestamp: string; blocked: boolean }[];
            }
        } catch {}

...
function activate(context) {
    ...
    // ── Register sidebar ──────────────────────────────────────────────────────
    const sidebarProvider = new sidebarProvider_1.AgenticanaSidebarProvider(context.extensionUri);
    context.subscriptions.push(vscode.window.registerWebviewViewProvider('agenticana.sidebarView', sidebarProvider, { webviewOptions: { retainContextWhenHidden: true } }));
    // ── Helper: resolve project root ──────────────────────────────────────────
    function getProjectRoot() {
        const cfg = vscode.workspace.getConfiguration('agenticana');
        const customRoot = cfg.get('projectRoot', '');
        if (customRoot)
            return customRoot;
    ...
    function runInTerminal(label, cmd, cwd) {
        const terminal = vscode.window.createTerminal({ name: `Agenticana — ${label}`, cwd });
        terminal.show();
        terminal.sendText(cmd);
    }
    // ── Commands ──────────────────────────────────────────────────────────────
    context.subscriptions.push(vscode.commands.registerCommand('agenticana.openSidebar', () => {
        vscode.commands.executeCommand('agenticana.sidebarView.focus');
    }));
    context.subscriptions.push(vscode.commands.registerCommand('agenticana.refresh', () => {
        sidebarProvider.refresh();
        vscode.window.showInformationMessage('🦅 Agenticana sidebar refreshed.');
    }));
    context.subscriptions.push(vscode.commands.registerCommand('agenticana.guardianStatus', async () => {
        const root = getProjectRoot();
        const py = getPython();
        runInTerminal('Guardian Status', `${py} scripts/guardian_mode.py status`, root);
    }));
    context.subscriptions.push(vscode.commands.registerCommand('agenticana.runNLSwarm', async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('agenticana.sovereignScan', async () => {
        const root = getProjectRoot();
        const py = getPython();
        const cfg = vscode.workspace.getConfiguration('agenticana');
    ...
    context.subscriptions.push(vscode.commands.registerCommand('agenticana.sovereignEvolve', async () => {
        const mode = await vscode.window.showQuickPick(['Full evolve (commit + push)', 'Evolve without push (--no-push)', 'Dry run only'], { title: '🦅 Sovereign Loop — Select mode' });
        if (!mode)
            return;
        const root = getProjectRoot();
        const py = getPython();
        let flags = '';
        if (mode.includes('Dry run'))
            flags = ' --dry-run';
        else if (mode.includes('without push'))
            flags = ' --no-push';
        runInTerminal('Sovereign Loop', `${py} scripts/sovereign_loop.py${flags}`, root);
    }));
    context.subscriptions.push(vscode.commands.registerCommand('agenticana.powSign', async () => {
        const root = getProjectRoot();
        const py = getPython();
        runInTerminal('Proof-of-Work Sign', `${py} scripts/pow_commit.py sign`, root);
    }));
    context.subscriptions.push(vscode.commands.registerCommand('agenticana.openDashboard', async () => {
        const cfg = vscode.workspace.getConfiguration('agenticana');
        const port = cfg.get('dashboardPort', 8080);
        const url = `http://127.0.0.1:${port}`;
        const choice = await vscode.window.showInformationMessage(`Open Agenticana Dashboard at ${url}?`, 'Open in Browser', 'Start Dashboard Server');
